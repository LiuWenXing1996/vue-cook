...
import useAgeRange from '@/../demos/test-pkg/logic/fetch-with-state/useAgeRange';
...
watchEffect(() => {
    ...
    useAgeRange(cookPlayerState).setMaxAge(maxAgeNumber)
})
watchEffect(() => {
    const minAgeNumber = Number(minAge?.value) || undefined
    useAgeRange(cookPlayerState).setMinAge(minAgeNumber)
})
interface IDataItem {
    name: string,
    age: number
